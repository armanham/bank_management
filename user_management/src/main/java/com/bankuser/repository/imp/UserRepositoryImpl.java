...
public class UserRepositoryImpl extends SimpleJpaRepository<UserEntity, Long> implements UserRepository {
    ...
    public ResponseEntity <UserP> update (final UserP userP) {
        ...
        if (user.isPresent()){
            if (userP.getAddressP() != null){
                entityManager.createQuery("UPDATE UserEntity u SET u.address = :newaddress WHERE u.username = :username")
                        .setParameter("newaddress", new Address(userP.getAddressP()))
                        .setParameter("username", user.get().getUsername())
                        .executeUpdate();
            }
            if (userP.getEmail() != null){
                entityManager.createQuery("UPDATE UserEntity u SET u.email = :newemail WHERE u.username = :username")
                        .setParameter("newemail", userP.getEmail())
                        .setParameter("username", user.get().getUsername())
                        .executeUpdate();
            }
            if (userP.getUsername() != null){
                entityManager.createQuery("UPDATE UserEntity u SET u.username = :newusername WHERE u.username = :username")
                        .setParameter("newusername", userP.getUsername())
                        .setParameter("username", user.get().getUsername())
                        .executeUpdate();
            }
            if (userP.getPhoneNumber() != null){
                entityManager.createQuery("UPDATE UserEntity u SET u.phoneNumber = :newphoneNumber WHERE u.username = :username")
                        .setParameter("newphoneNumber", userP.getPhoneNumber())
                        .setParameter("username", user.get().getUsername())
                        .executeUpdate();
            }
            return new ResponseEntity<>(HttpStatusCode.valueOf(200));
    ...
    public ResponseEntity <UserP> getUserPResponseEntity (String password, Optional <UserEntity> user) {
        if (user.isPresent()) {
            entityManager.createQuery("UPDATE UserEntity u SET u.passwordHash = :newPasswordHash WHERE u.username = :username")
